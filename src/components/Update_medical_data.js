...
import { View,SafeAreaView,ScrollView, TouchableOpacity , Text, TextInput, Button, Image, StyleSheet } from 'react-native';
...
const Update_medical_data= ({route, navigation }) => {// func declaration argugemnt navigation  function adddogs (navigation) {}
  const { dog } = route.params;
  var tempAlergies,tempMedications,tempTreatments;
  var tempRabiesVaccine,tempChip,tempCanineHepatitis,tempSpirocercaLupi,tempCastration,tempDeworming,tempfleaTreatment;
  const [rabiesVaccineDate, setRabiesVaccine] = React.useState(new Date());
  const [chipDate, setChip] = React.useState(dog.chip);
  const [canineHepatitisDate, setCanineHepatitis] = React.useState(dog.canineHepatitis);
  const [spirocercaLupiDate, setSpirocercaLupi] = React.useState(dog.spirocercaLupi);
  ...
  const [fleaTreatmentDate,setfleaTreatment] = React.useState(dog.fleaTreatment);
  ...
  const YesNoArray = [// array with two arguments
    { label: 'Yes', value: 'No'},
    { label: 'Yes', value: 'No'},
    ]
    ...
    return(
      ...
      <Text style={styles.radioButtonText}>Choose canine hepatitis Date:</Text>
      <TouchableOpacity style={styles.loginButton} onPress={showDateTimePicker}>
        <Text style={styles.buttonText}>Choose date</Text>
      </TouchableOpacity>
      {showPicker && (
...
<Text style={styles.radioButtonText}>Choose deworming Date:</Text>
    ...
    >
      ...
      <TextInput style={styles.input} value={alergies} onChangeText={onChangeAlergies} placeholder='Alergies:' placeholderTextColor="#8B5A33"/>
      <TextInput style={styles.input} value={medications} onChangeText={onChangeMedications} placeholder='Medical :' placeholderTextColor="#8B5A33"/>
      <TextInput style={styles.input} value={medicalTreatment} onChangeText={onChangeMedicalTreatment} placeholder='Medical Treatment:' placeholderTextColor="#8B5A33"/>
